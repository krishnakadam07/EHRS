...
import { FiMaximize, FiShield, FiAlertCircle, FiCheckCircle, FiX } from 'react-icons/fi';
...
export default function ScanQR() {
  ...
  useEffect(() => {
    // We only mount the scanner if scanning is true and scannedData is null
    ...
    const startScanner = async () => {
      try {
        ...
        await html5QrCode.start(
          ...
          () => {
            // Error Callback - Triggers every frame it fails to find a QR, ignore to prevent spam
          }
        );
      } catch (err) {
  ...
  const handleAccessProfile = () => {
    // Navigate to Emergency View, passing the patient ID in state
    navigate(ROUTES.PUBLIC.EMERGENCY_VIEW, { state: { patientId: scannedData.patientId, verified: true } });
  };
  ...
  return (
    <div className="flex flex-col gap-6 pb-12 max-w-4xl mx-auto h-full">
      ...
      <Modal
        isOpen={!!scannedData}
        onClose={() => { setScannedData(null); setIsScanning(true); }}
        size="md"
      >
        {scannedData && (
