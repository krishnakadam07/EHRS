...
export default function useAuth() {
  ...
  const login = useCallback(async (email, password, role) => {
    try {
      return await dispatch(loginUser({ email, password, role })).unwrap();
    } catch (err) {
      return false;
    }
  }, [dispatch]);
  ...
  // 🌟 FIX: Force lowercase immediately so the entire app works flawlessly!
  const userRole = (currentUser?.role || 'patient').toLowerCase();
